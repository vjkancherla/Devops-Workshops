...
echo "<<=============================>"
echo ""
echo "-->Running Shell commands, using SSM, to do additonal config on the Monitoring instance"
echo ""
...
while true:
do
  cmd_status=`aws ssm list-command-invocations --command-id ${cmd_id} --query "CommandInvocations[0].StatusDetails"`
  if [ ${cmd_status} -eq "In Progress" ]
  then
    if [ ${i} -lt 24 ]
    then
      echo "-->SSM command is still 'In Progress'. Will wait for 120 secs for it to complete. So far, $((i*5)) secs have elapsed"
      sleep 5
      i=$((i+1))
      continue
    else
      echo "SSM command did not complete in 120 secs. Exiting"
      rm ssm.log
      exit 1
    fi
  elif [ ${cmd_status} -eq "Failed" ]
    then
      echo "-->SSM command failed. Exiting"
      rm ssm.log
      exit 1
  elif [ ${cmd_status} -eq "Success" ]
    then
      echo "-->SSM command successfully complete"
      rm ssm.log
      break
  fi
done
