...
retry=true
do_Additional_Config_On_Monitoring_Instance() {
  echo "<<=============================>"
  echo ""
  echo "-->Running Shell commands, using SSM, to do additional config on the Monitoring instance"
  echo ""

  echo "Waiting for 60 seconds, to allow Monitoring instance to complete bootstrapping. Starting to sleep at `date`"
  sleep 60
  echo "Sleep done"
  ...
  do
    ...
    if [ "${cmd_status}" == "InProgress" ]
      ...
      then
        if $retry
        then
          echo "-->SSM command failed, RETRYING..."
          retry=false
          do_Additional_Config_On_Monitoring_Instance
        else
          echo "-->SSM command failed, TWICE. Exiting"
          rm ssm.log
          exit 1
        fi
